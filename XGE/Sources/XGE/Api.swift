...
public class Api {
    ...
    private var _me:Node?
    private var _stateNode:Node?
    ...
    public func setScriptNode(node:Node) {
        self._current = node
        self._me = node
        self._graphic = nil
    }
    
    public func setInitState(isInit:Bool) {
        self._init = isInit
        self._time = 0
    }
    
    public func setGameState() {
        self._stateNode = Node()
        self._collider = Collider()
    }
    
    public func initApi(node:Node) -> Void {
        
        if node.context == nil {
            node.context = JSContext()
        }
        
        if let context = node.context {
            context.exceptionHandler = { context, exception in
                if let exception = exception {
                    Log.instance.put("ERROR - \(node.name).js")
            ...
            // inDesign()
            // return true if the script is running in design (in the scene editor) and false otherwise
            let inDesign: @convention(block) () -> Bool = { GameView.inDesign }
            context.setObject(inDesign, forKeyedSubscript: "inDesign" as NSString)
            ...
            // loadScene(name)
            // load a scene with the given file name
            let loadScene: @convention(block) (String) -> Void = { name in
                if !GameView.inDesign {
                    GameView.instance!.scene.loadSceneName = name
                }
            }
            context.setObject(loadScene, forKeyedSubscript: "loadScene" as NSString)
            
            // sceneName()
            // return the file name of the current loaded scene
            let sceneName: @convention(block) () -> String = { GameView.instance!.scene.name }
            context.setObject(sceneName, forKeyedSubscript: "sceneName" as NSString)
            ...
            // toMe()
            // set the current node to this script's root node
            let toMe: @convention(block) () -> Void = { [weak self] in self!._current = self!._me }
            ...
            // toState()
            // set the current node to the global state node, the state node is created just once, when a game starts running
            let toState: @convention(block) () -> Void = { [weak self] in self!._current = self!._stateNode }
            context.setObject(toState, forKeyedSubscript: "toState" as NSString)
