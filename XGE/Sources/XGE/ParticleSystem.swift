...
#else
        _vertexBuffer = GameView.instance!.device!.makeBuffer(length: MemoryLayout<Vertex>.stride * 6 * maxParticles, options: .storageModeShared)
#endif
    }
...
#else
        _vertexBuffer = GameView.instance!.device!.makeBuffer(length: MemoryLayout<Vertex>.stride * 6 * _maxParticles, options: .storageModeShared)
#endif
    }
